<template>
	<div class="admin-check">
		<div class="first">
		<h4>图片审核通过录入</h4>
		打卡人QQ号<input v-model="QQ" placeholder="打卡人QQ号"/><br>
		打卡任务的id<input v-model="task" placeholder="打卡任务id" /><br>
		<button @click="click1">将该条数据录入数据库</button>
		</div>
		<div class="second">
		<h4>当日所有任务全部审核完成后，点击此按钮</h4>
		<button @click="click2">更新数据</button>
...
<script>
	import {record,update} from '../../network/admin.js'
	import {Message} from 'element-ui'
	export default{
		name:'admin-check',
		methods:{
			click1(){
				record(this.QQ,this.task).then( res => {
					if(res.success){
				  Message({
				             showClose: true,
				             message: '操作成功',
